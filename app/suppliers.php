...
$importPreview = null;if ($isAdmin && isset($_GET['import_preview'])) {    $importPreview =        $_SESSION['supplier_import_preview'] ?? null;}$search = trim($_GET['search'] ?? '');
    ...
    <?php if (($importPreview['valid'] ?? 0) > 0): ?>
        ...
        <div style="margin-top:16px;padding:14px;background:#f0fdf4;border:1px solid #86efac;border-radius:8px;">
            ...
            <div style="margin-bottom:10px;color:#166534;font-weight:bold;">
                Có <?= (int)$importPreview['valid'] ?> nhà cung cấp hợp lệ, sẵn sàng nhập.
            </div>

            <form
                method="POST"
                action="import_suppliers_commit.php"
                onsubmit="return confirm('Bạn có chắc muốn nhập <?= (int)$importPreview['valid'] ?> nhà cung cấp hợp lệ vào database?');"
            >
